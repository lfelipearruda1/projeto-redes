...
def main():
    ...
    while True:
        ...
        if ptype == "SEND_START":
            ...
            partes = {}
            reject_current = text_len < MIN_MSG_LEN
            status = "rejected_min_len" if reject_current else "ready"
            print(f"\n[RECV] SEND_START msg_id={current_id} text_len={text_len} status={status}")
            ...
            if reject_current:
                send_json(conn, {"type":"ACK","msg_id":msg_id,"seq":seq,"status":"rejected_min_len"})
                continue

            partes[seq] = payload
            csum = checksum8(payload)
            print(f"[PKT] msg_id={msg_id} seq={seq}/{total-1} payload_len={len(payload)} checksum={csum} payload='{payload}'")
